using System;
using System.ComponentModel;
using System.Linq;
using System.Reactive.Linq;
using System.Collections.Generic;
using static TorchSharp.torch;

namespace Bonsai.ML.Torch
{
    /// <summary>
    /// Stacks tensors along a new dimension. All tensors need to be of the same size.
    /// </summary>
    [Combinator]
    [Description("Stacks tensors along a new dimension. All tensors need to be of the same size.")]
    [WorkflowElementCategory(ElementCategory.Transform)]
    public class Stack
    {
        /// <summary>
        /// The new dimension along which to concatenate the tensors.
        /// </summary>
        [Description("The new dimension along which to concatenate the tensors.")]
        public long Dimension { get; set; } = 0;

        /// <summary>
        /// Stacks the input tensors along the new dimension.
        /// </summary>
        public IObservable<Tensor> Process(IObservable<Tuple<Tensor, Tensor>> source)
        {
            return source.Select(value => stack([value.Item1, value.Item2], Dimension));
        }

        /// <summary>
        /// Stacks the input tensors along the new dimension.
        /// </summary>
        public IObservable<Tensor> Process(IObservable<Tuple<Tensor, Tensor, Tensor>> source)
        {
            return source.Select(value => stack([value.Item1, value.Item2, value.Item3], Dimension));
        }

        /// <summary>
        /// Stacks the input tensors along the new dimension.
        /// </summary>
        public IObservable<Tensor> Process(IObservable<Tuple<Tensor, Tensor, Tensor, Tensor>> source)
        {
            return source.Select(value => stack([value.Item1, value.Item2, value.Item3, value.Item4], Dimension));
        }

        /// <summary>
        /// Stacks the input tensors along the new dimension.
        /// </summary>
        public IObservable<Tensor> Process(IObservable<Tuple<Tensor, Tensor, Tensor, Tensor, Tensor>> source)
        {
            return source.Select(value => stack([value.Item1, value.Item2, value.Item3, value.Item4, value.Item5], Dimension));
        }

        /// <summary>
        /// Stacks the input tensors along the new dimension.
        /// </summary>
        public IObservable<Tensor> Process(IObservable<Tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor>> source)
        {
            return source.Select(value => stack([value.Item1, value.Item2, value.Item3, value.Item4, value.Item5, value.Item6], Dimension));
        }

        /// <summary>
        /// Stacks the input tensors along the new dimension.
        /// </summary>
        public IObservable<Tensor> Process(IObservable<Tuple<Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor>> source)
        {
            return source.Select(value => stack([value.Item1, value.Item2, value.Item3, value.Item4, value.Item5, value.Item6, value.Item7], Dimension));
        }

        /// <summary>
        /// Stacks the input tensors along the new dimension.
        /// </summary>
        public IObservable<Tensor> Process(IObservable<IEnumerable<Tensor>> source)
        {
            return source.Select(value => stack([.. value], Dimension));
        }
    }
}
